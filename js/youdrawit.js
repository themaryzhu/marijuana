// Code from https://github.com/EE2dev/you-draw-it

(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('d3v5')) :
  typeof define === 'function' && define.amd ? define(['exports', 'd3'], factory) :
  (factory((global.youdrawit = {}),global.d3v5));
}(this, (function (exports,d3v5) { 'use strict';
  ...
  function getScore(key, truth$$1, state, graphMaxY, graphMinY, resultSection, scoreTitle, scoreButtonText, scoreButtonTooltip, scoreHtml) {
    ...
    var scoreFunction = d3v5.scaleLinear().domain([0, maxDiff]).range([100, 0]);
  ...
  function getFinalScore(key, state, resultSection, scoreTitle, scoreButtonText, scoreButtonTooltip, scoreHtml) {
    ...
    if (completed$$1) {
      ...
      var finalScoreFunction = d3v5.scaleLinear().domain([0, 100 * state.getAllQuestions().length]).range([0, 100]);
  ...
  function drawScore(finalScore, resultSection, key, scoreTitle, scoreButtonText, scoreButtonTooltip, scoreHtml) {
    ...
    button.on("click", function () {
      d3v5.select("div.actionContainer.finalScore").style("display", "none");
  ...
  function showFinalScore(finalScore, resultSection, key, svgWidth) {
    ...
    function showText() {
      d3v5.select(".result." + key).select("text.scoreText").style("opacity", 1);
    ...
    var xScale = d3v5.scaleLinear().domain([0, 100]).range([0, svgWidth - 80]);
    var xAxis = d3v5.axisBottom(xScale).ticks(4);
  ...
  function addReferenceLines(sel, svg, referenceValues, c) {
    var gRef = void 0;
    var data = void 0;
    var referenceLine = d3v5.line().x(ƒ("year", c.x)).y(ƒ("value", c.y)).curve(d3.curveMonotoneX);
  ...
  function ydLine(isMobile, state, sel, key, question, globals, data, indexedTimepoint, indexedData) {
    ...
    var minY = d3v5.min(data, function (d) {
      return d.value;
    });
    var maxY = d3v5.max(data, function (d) {
      return d.value;
    });
    ...
    var drawChart = function drawChart(lower, upper, addClass) {
      ...
      var area = d3v5.area().curve(d3v5.curveMonotoneX).x(ƒ("year", c.x)).y0(ƒ("value", c.y)).y1(c.height).defined(definedFn);
      var line = d3v5.area().curve(d3v5.curveMonotoneX).x(ƒ("year", c.x)).y(ƒ("value", c.y)).defined(definedFn);
    ...
    c.x = d3v5.scaleLinear().range([0, c.width]);
    c.x.domain([minX, maxX]);
    c.y = d3v5.scaleLinear().range([c.height, 0]);
    ...
    // gradients
    c.defs = d3v5.select(c.svg.node().parentNode).append("defs");
    ...
    // configure axes
    c.xAxis = d3v5.axisBottom().scale(c.x);
    c.xAxis.tickFormat(function (d) {
      return indexedTimepoint[d];
    }).ticks(maxX - minX);
    c.yAxis = d3v5.axisLeft().scale(c.y); //.tickValues(c.y.ticks(6));
    ...
    // add preview wave
    var arc = d3v5.arc().startAngle(0).endAngle(Math.PI);
    ...
    function moveWave() {
      ...
      c.wave.style("opacity", .6).transition().ease(d3v5.easeLinear).delay(function (d, i) {
        return 1000 + i * 300;
    ...
    function arcTween() {
      return function (d) {
        ...
        var interpolate = d3v5.interpolate(0, 100);
        return function (t) {
    ...
    var userLine = d3v5.line().x(ƒ("year", c.x)).y(ƒ("value", c.y)).curve(d3v5.curveMonotoneX);
    ...
    var resultSection = d3v5.select(".result." + key);
    ...
    var interactionHandler = function interactionHandler() {
      ...
      if (!state.get(key, completed) && d3v5.mean(state.get(key, yourData), ƒ("defined")) == 1) {
    ...
    c.svg.call(d3v5.drag().on("drag", interactionHandler));
  ...
  function ydBar(isMobile, state, sel, key, question, globals, data, indexedTimepoint, indexedData) {
    ...
    var graphMinY = question.yAxisMin ? question.yAxisMin : minY >= 0 ? 0 : minY * getRandom(1, 1.5);
    var graphMaxY = question.yAxisMax ? question.yAxisMax : maxY + (maxY - graphMinY) * getRandom(0.4, 1); // add 40 - 100% for segment titles
    c.x = d3v5.scaleBand().rangeRound([0, c.width]).padding(0.1);
    ...
    c.grid.append("g").attr("class", "vertical").call(d3v5.axisLeft(c.y).tickValues(c.y.ticks(6)).tickFormat("").tickSize(-c.width));
    ...
    // add preview wave
    var arc = d3v5.arc().startAngle(0).endAngle(2 * Math.PI);
  ...
  */
  ...
  function ydCheckbox(isMobile, state, sel, key, question, globals, data) {
    ...
    function handleClick() {
      ...
      var index = d3v5.select(this).attr("value").substring(1);
      console.log("Clicked, new value [" + index + "] = " + d3v5.select(this).node().checked);
    ...
    function showResultChart() {
      ...
      sel.selectAll("div input").each(function () {
        d3v5.select(this).node().disabled = true;
      });
      ...
      data.forEach(function (ele, i) {
        ...
        sel.select("div span.answer-checkmark-truth.t-" + i).classed("checked", ele.value).style("background-color", "#fff").transition().ease(ele.value ? d3v5.easeExpIn : d3v5.easeLinear).duration(100).delay(i * 100).style("background-color", ele.value ? "#00345e" : "#eee");
  ...
  function youdrawit(_globals, questions) {
    ...
    var drawGraphs = function drawGraphs() {
      d3v5.selectAll(".you-draw-it").each(function (d, i) {
        var sel = d3v5.select(this);
  ...
  function _interface () {
    ...
    var options = {};
    options.containerDiv = d3v5.select("body");
    options.globals = {};
    ...
    */
    ...
    function chartAPI(selection) {
      selection.each(function () {
        options.containerDiv = d3v5.select(this);
